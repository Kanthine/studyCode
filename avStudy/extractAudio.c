...
int main(int argc, char *argv[]) {
    ...
    const char *src_filename = argv[1]; /// 多媒体文件
    const char *dst_filename = argv[2]; /// 输出的音频地址
    ...
    AVFormatContext *fmt_ctx = NULL; /// 多媒体上下文
    AVFrame *frame = NULL;
    AVPacket pkt;
    ...
    */
    ...
    int channels    = fmt_ctx->streams[audio_stream_index]->codecpar->ch_layout.nb_channels;
    int sample_rate = fmt_ctx->streams[audio_stream_index]->codecpar->sample_rate;
    enum AVCodecID codec_id = fmt_ctx->streams[audio_stream_index]->codecpar->codec_id;
    ...
    while(av_read_frame(fmt_ctx, &pkt) >=0 ){
        if(pkt.stream_index == audio_stream_index){
            ...
            write_len = fwrite( pkt.data, 1, pkt.size, dst_fd);
            if(write_len != pkt.size){
