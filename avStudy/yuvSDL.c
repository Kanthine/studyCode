...
int main(int argc, char* argv[]) {
    ...
    /// 单独开启一个线程：用于控制每秒播放的帧数
    SDL_Thread *timer_thread = NULL;
    
    int w_width = 1080, w_height = (1152 / 2028.0) * 1080;  /// 屏幕大小
    const int video_width = 2028, video_height = 1152; /// 源 YUV 的分辨率
    ...
    const char *path = "/Users/i7y/Desktop/1.yuv";
    ...
    //IYUV: Y + U + V  (3 planes)
    //YV12: Y + V + U  (3 planes)
    pixformat= SDL_PIXELFORMAT_IYUV; /// 像素格式为 YUV

    //create texture for render
    texture = SDL_CreateTexture(renderer,
                                pixformat,
                                SDL_TEXTUREACCESS_STREAMING, /// 设置纹理为 数据流
                                video_width,
                                video_height);
    ...
    do {
        //Wait
        SDL_WaitEvent(&event);
        if(event.type == REFRESH_EVENT) { /// 收到刷新事件
