...
struct SkyboxVertex {
    float4 position [[attribute(VertexInputVertex)]];
};
...
vertex FPData
vertexShaderFP(SkyboxVertex             in        [[ stage_in ]],
               constant vector_float2   &viewportSize [[buffer(VertexInputViewportSize)]],
               constant int64_t &frameTime [[buffer(VertexInputFrametime)]]) {
    
    FPData out;

    float2 position = in.position.xy;
    
    float mask_x = in.position.x;
    float mask_y_top = 5 * sin(mask_x / 180.0 * 3.1415926 * KPointSize / 8.0) - frameTime + KPointSize / 2.0;
    float mask_y_bottom = 5 * sin(mask_x / 180.0 * 3.1415926 * KPointSize / 8.0) + frameTime - KPointSize / 2.0;
    if (in.position.y < mask_y_top && in.position.y > mask_y_bottom) {
        out.isMask = false;
    } else {
        out.isMask = true;
    }
    ...
    if (out.isMask == true) {
        float2 rate = float2(random(position.xy + KPointSize / 2) * 6 - 3, random(position.yx + KPointSize / 2) * 6 - 3);
        rate = abs(rate);
        for( int i = 0; i < frameTime; i++) {
            float offet = snoise(float3(position.x / 100.0, position.y / 100.0, frameTime / 100.0));
            position += rate + offet;
            if (position.x < -viewportSize.x / 3.0 || position.x > viewportSize.x / 3.0 ||
                position.y < -viewportSize.y / 3.0 || position.y > viewportSize.y / 3.0) {
                position = in.position.xy;
            }
        }
    }
    
    out.position.x = position.x / viewportSize.x * 2.0;
    out.position.y = position.y / viewportSize.y * 2.0;
    out.position.z = 0;
    out.position.w = 1.0;
    out.pointSize = 1.0;
    return out;
