...
private:
    ...
    static float lightLocation[3]; //定位光源位置
public:
    
    static GLfloat* cameraFB;
    static GLfloat* lightPositionFB;
    ...
    static void setProjectOrtho(float left, float right, float bottom, float top, float near, float far) {
        GLMatrix::orthoM(mProjMatrix, 0, left, right, bottom, top, near, far);
    }
