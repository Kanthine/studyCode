...
vertexShaderFP(SkyboxVertex             in        [[ stage_in ]],
               ...
               constant PEUniform &uniforms [[buffer(VertexInputUniform)]]) {
    
    float4 position = in.position;
    float3 noise = curlNoise(float3(position.x*.02,position.y*.008,uniforms.frameTime * 0.05));
    float3 distortion = float3(position.x*2.,position.y,1.) * noise * uniforms.uProgress;
    float4 newPos = position + float4(distortion, 1.0);
        
    FPData out;
    out.position.x = newPos.x / viewportSize.x * 2.0;
    out.position.y = newPos.y / viewportSize.y * 2.0;
    out.position.z = newPos.z / 300.0;
    out.position.w = 1.0;
    out.pointSize = 2.0;
    return out;
}
