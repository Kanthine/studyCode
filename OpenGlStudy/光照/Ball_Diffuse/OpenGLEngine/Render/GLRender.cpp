...
void GLRender::Initialize(int width, int height) {//初始化函数
    ...
    GLMatrixState::setCamera(0, 0, 15, 0, 0, 0, 0, 1, 1);//设置摄像机矩阵
...
void GLRender::Render() const {
    ...
    GLMatrixState::setLightLocation(0, lightOffset, 1.5f); //设置光源初始位置
    GLMatrixState::pushMatrix();//保护现场
    GLMatrixState::rotate(eAngle, 0, 1, 0);//地球自转
    GLMatrixState::translate(0, 1.3, 0);
    earth->drawSelf();//绘制地球
    ...
    eAngle = eAngle + 0.2f;//地球继续旋转一定的角度
}
