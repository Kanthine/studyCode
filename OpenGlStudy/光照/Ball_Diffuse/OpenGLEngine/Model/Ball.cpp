...
void Ball::initVertexData() {
    ...
    /// 由于本案例中球心位于坐标原点，所以每个顶点法向量的 x、y、z 轴分量与顶点 的 x、y、z 坐标是一致的。
    /// 这样就不必单独计算每个顶点的法向量了，直接将顶点坐标序列看作 顶点法向量序列使用即可。
    mNormalBuffer = ver;
}

void Ball::initShader() {
    mProgram = ShaderTool::createProgram("VertexShader", "FragmentShader");
    maPositionHandle = glGetAttribLocation(mProgram, "aPosition"); /// 获取程序中顶点位置属性引用
    muMVPMatrixHandle = glGetUniformLocation(mProgram, "uMVPMatrix"); /// 获取程序中总变换矩阵引用
    muMMatrixHandle = glGetUniformLocation(mProgram, "uMMatrix");
    muRHandle = glGetUniformLocation(mProgram, "uR"); /// 获取程序中球半径的引用
    maNormalHandle= glGetAttribLocation(mProgram, "aNormal"); // 获取程序中顶点法向量属性引用
    maLightLocationHandle = glGetUniformLocation(mProgram, "uLightLocation"); //获取程序中光源位置引用
}
...
void Ball::drawSelf() {
    ...
    glUniform1f(muRHandle, r); // 将半径属性传入渲染管线
    glUniform3fv(maLightLocationHandle, 1, GLMatrixState::lightPositionFB); //将光源位置数据传入渲染管线
    glVertexAttribPointer(maPositionHandle,3, GL_FLOAT, GL_FALSE, 3 * 4, mVertexBuffer); //将顶点位置数据送入渲染管线
    glVertexAttribPointer(maNormalHandle, 3, GL_FLOAT, GL_FALSE, 3 * 4, mNormalBuffer); //将顶点法向量数据传入渲染管线
    glEnableVertexAttribArray(maPositionHandle); //启用顶点位置数据数组
    glEnableVertexAttribArray(maNormalHandle); //启用顶点位置数据数组
