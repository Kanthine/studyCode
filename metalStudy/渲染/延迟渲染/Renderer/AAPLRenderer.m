...
/// 在延迟渲染中绘制平行光：使用模板测试剔除无效像素，只渲染需要被照射的像素
...
/**
 * 在 3D 空间中共同的多边形可以通过向每一个空间添加 z 偏差 (或深度偏差) 来测试它们是否不是共同面。
 * 这是一种通常用于确保正确显示场景中的阴影的技术。
 * 例如，墙上的阴影可能具有与墙相同的深度值。 如果应用程序先呈现墙，然后呈现阴影，则阴影可能不可见，或者深度项目可能可见。
 *
 * 应用程序可以通过将 depthBias 成员的偏差 (添加到系统在呈现共同平面多边形集时使用的 z 值来帮助确保正确呈现平面多边形。
 * 具有较大 z 值的多边形将在多边形前面绘制，其 z 值较小。
 *
 * 计算深度偏差有两个选项：
 *  1、如果当前绑定到输出合并阶段的深度缓冲区具有 UNORM 格式或未绑定深度缓冲区，则偏差值将如下所示进行计算：
 *      Bias = (float)DepthBias * r + SlopeScaledDepthBias * MaxDepthSlope;
 *    其中 r 是深度缓冲区格式的最小可表示值 > 0，转换为 float32。
 *    DepthBias 和 SlopeScaledDepthBias 值D3D11_RASTERIZER_DESC1结构成员。
 *    MaxDepthSlope 值是像素深度值的水平斜率和垂直斜率的最大值。
 *  2、如果浮点深度缓冲区绑定到输出合并阶段，则会按如下所示计算偏差值：
 *      Bias = (float)DepthBias * 2*(exponent(max z in primitive) - r) + SlopeScaledDepthBias * MaxDepthSlope;
 *    其中 r 是浮点表示形式的 mantissa 位数， (排除隐藏位) ;例如，23 for float32。
 *
 * 然后，偏差值将如下所示进行固定：
 *      if(DepthBiasClamp > 0)
 ...
 *             z = z + Bias
 * 剪裁后对顶点进行深度偏差运算，因此深度偏差对几何剪裁没有影响。 偏差值是给定基元的常量，并在内插器设置之前添加到每个顶点的 z 值。 使用 特征级别 10.0 及更高时，将使用 32 位浮点算术执行所有偏差计算。 偏差不适用于任何点或线基元，但线框模式下绘制的线条除外。
 *
 * 其中一个具有基于阴影缓冲区的阴影是阴影缓冲区的项目，或者由于着色器中深度计算与阴影缓冲区中同一表面的深度差异而产生表面阴影本身。 缓解此问题的一种方法是在呈现阴影缓冲区时使用 DepthBias 和 SlopeScaledDepthBias 。 该想法是在呈现阴影缓冲区时将图面推送到足够多，以便阴影缓冲区 z 和着色器 z) 之间的比较结果 (在图面上保持一致，并避免本地自我阴影。
 *
 * 但是，如果使用 DepthBias 和 SlopeScaledDepthBias ，当以极其尖锐的角度查看的多边形会导致偏差公式生成非常大的 z 值时，可能会引入新的呈现问题。 这实际上使多边形与阴影图中的原始表面非常远。 帮助缓解此特定问题的一种方法是使用 DepthBiasClamp，它提供对计算 z 偏差幅度的上限 (正或负) 。
 *
