...
averageResolveKernel(texture2d_ms<float, access::read> multisampledTexture [[texture(0)]],
    ...
    float4 color0 = multisampledTexture.read(gid, 0);
    float4 color1 = multisampledTexture.read(gid, 1);
    float4 color2 = multisampledTexture.read(gid, 2);
    float4 color3 = multisampledTexture.read(gid, 3);
