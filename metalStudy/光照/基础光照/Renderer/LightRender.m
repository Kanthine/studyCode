...
- (void)loadMetalAsset {
    ...
    #pragma mark Load textures for non-mesh assets
    {
        ...
        _modelMap = [textureLoader newTextureWithName:@"earth"
                                        scaleFactor:1.0
                                             bundle:nil
                                            options:textureLoaderOptions
                                              error:&error];
        NSAssert(_modelMap, @"Could not load model texture: %@", error);
        _modelMap.label = @"Sky Map";
    }
}
...
- (void)updateSceneState {
    ...
    _uniforms.lightLocation = simd_make_float3(2.0, 0.0, -2.0);
    _uniforms.lightDirection = simd_make_float3(2.0, 0.0, -2.0);
    
    _uniforms.worldMatrix =  matrix_multiply(matrix4x4_rotation(-45 / 360.0, 0, 0, 1), matrix4x4_rotation(_rotation, 0, 1, 0));
}
...
- (void)drawSky:(nonnull id <MTLRenderCommandEncoder>)renderEncoder {
    ...
    [renderEncoder setFragmentTexture:_modelMap atIndex:kAttributeTexture];
...
- (void)mtkView:(nonnull MTKView *)view drawableSizeWillChange:(CGSize)size {
    ...
    _uniforms.projectionMatrix = matrix_perspective_left_hand(65.0f * (M_PI / 180.0f), aspect, 0.1, 50);
    _uniforms.cameraPos = simd_make_float3(0, 0, -2.5);
...
- (void)setLightType:(kLightType)lightType {
    ...
    switch (lightType) {
        case kLightTypeAmbient:{
            renderPipelineDescriptor.vertexFunction = [defaultLibrary newFunctionWithName:@"vertexRender_Ambient"];
            renderPipelineDescriptor.fragmentFunction = [defaultLibrary newFunctionWithName:@"fragmentShader_Ambient"];
        }break;
        case kLightTypeDiffuse:{
            renderPipelineDescriptor.vertexFunction = [defaultLibrary newFunctionWithName:@"vertexRender_Diffuse"];
            renderPipelineDescriptor.fragmentFunction = [defaultLibrary newFunctionWithName:@"fragmentShader_Diffuse"];
        }break;
        case kLightTypeSpecular:{
            renderPipelineDescriptor.vertexFunction = [defaultLibrary newFunctionWithName:@"vertexRender_Specular"];
            renderPipelineDescriptor.fragmentFunction = [defaultLibrary newFunctionWithName:@"fragmentShader_Specular"];
        }break;
        case kLightTypeCompound:{
        ...
        default:{
            renderPipelineDescriptor.vertexFunction = [defaultLibrary newFunctionWithName:@"vertexRender"];
            renderPipelineDescriptor.fragmentFunction = [defaultLibrary newFunctionWithName:@"fragmentShader"];
        }break;
    }
