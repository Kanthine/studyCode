...
//        modelFileURL = [[NSBundle mainBundle] URLForResource:@"rw1" withExtension:@"obj"];
...
- (void)mtkView:(nonnull MTKView *)view drawableSizeWillChange:(CGSize)size {
    ...
    _uniforms.cameraPos = simd_make_float3(0, 100, -100);
    _uniforms.cameraMatrix = matrix_look_at_left_hand(_uniforms.cameraPos,
                                                      simd_make_float3(0, 0, 0),
                                                      simd_make_float3(0, 1, 0));
    _uniforms.projectionMatrix = matrix_perspective_left_hand(0.32 * M_PI, _viewportSize.x / _viewportSize.y, 1.0, 250.0f);
    ...
    _uniforms.lightLocation  = simd_make_float3(0, 0.0, -100);   /// 光源位置
    _uniforms.lightDirection = simd_make_float3(0, 0.0, -2.0);  /// 光源方向
}

#pragma mark - private method

- (void)updateGameState {
//    _uniforms.cameraPos = matrix_multiply(matrix4x4_rotation(_rotation, 0, 1, 0), simd_make_float4(0, 50, -100, 1)).xyz;
//    _uniforms.cameraMatrix = matrix_look_at_left_hand(_uniforms.cameraPos,
//                                                      simd_make_float3(0, 0, 0),
//                                                      simd_make_float3(0, 1, 0));
    _uniforms.worldMatrix = matrix4x4_identity();
    _rotation += 0.005f;
}
